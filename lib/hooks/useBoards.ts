  ...
  async function moveJob(
    ...
    setColumns((prev) => {
      const newColumns = prev.map((col) => ({
        ...col,
        jobApplication: [...col.jobApplication],
      }));
      ...
      for (const col of newColumns) { 
        const jobIndex = col.jobApplication.findIndex(
          (j) => j._id === jobApplicationId
        );
        if (jobIndex !== -1 && jobIndex !== undefined) {
          jobToMove = col.jobApplication[jobIndex];
          oldColumnId = col._id;
          col.jobApplication = col.jobApplication.filter(
      ...
      if (jobToMove && oldColumnId) {
        ...
        if (targetColumnIndex !== -1) {
          ...
          const currentJobs = targetColumn.jobApplication || [];
          ...
          newColumns[targetColumnIndex] = {
            ...targetColumn,
            jobApplication: jobsWithUpdatedOrders,
          };
        }
      }
