    ...
    <Card className="min-w-75 shrink-0 shadow-md p-0">
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className="rounded-md p-1 hover:bg-white/20">
...
export default function KanbanBoard({ board, userId }: KanbanBoardProps) {
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    const targetJob = sortedColumns
      .flatMap((col) => col.jobApplication || [])
    ...
    if (targetColumn) {
      ...
      const allJobsInTargetOriginal =
        targetColumnObj.jobApplication.sort((a, b) => a.order - b.order) || [];
  ...
  const activeJob = sortedColumns.flatMap((col) => col.jobApplication || []).find((job)=> job._id === activeId)
  return (
    <DndContext
      sensors={sensors}
